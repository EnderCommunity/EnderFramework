function installTheModules(tPath, id, modules, _callback_ = function(){}, installationProgress, installationMessage, appPaths){
  ...
  var modules = EnderFramework.parse(modules), newModules = [];
  for(var i = 0; i < modules.length; i++){
  ...
  var downloadLinks = [], Stop = false, Done_ = 0;
  const Done = () => {
    Done_++;
    ...
    if(Done_ == modules.length){
      ...
      //if(true){
        ...
        setTimeout(function(){
          var length = id.replace(/[^.]/g, "").length, newPath = path_.replace("built-in\\Installer\\", "") + "installed", done = 0, done_ = function(){
            done++;
            if(done == length){
                    ...
                    var done = 0, _done = function(){
                      ...
                      if(done == downloadLinks.length){
                        ...
                        function moveModuleFiles(fileName, callback = function(){}){
                          var exPath = tPath + "\\executable_" + num + ".bat", fileContent = "cd \"" + tPath + "\"\n";
                          num++;
                          //console.log(fileContent);
            ...
            const praseVersion = function(version, versions = null){
              if(!modules[i].strict){
                ...
                if(version.indexOf("=") > -1){
                  ...
                  if(versions !== null){
                    var closest = clear(version), done = false;
                    for(var vName in versions){
