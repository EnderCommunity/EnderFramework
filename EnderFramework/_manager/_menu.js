...
document.addEventListener("DOMContentLoaded", function(){
  (function(){
    ...
    if(_menuContent.type == "none"){
        ...
        for(var i in value){
          ...
          if(value[i].home == true){
            Element.addEventListener("click", function(){
              ...
              content.loadURL(appPath + "content/_main.html");
            });
            MainElement.insertAdjacentElement('beforeend', Element);
          }else if(value[i].link !== undefined){
            Element.url = ((value[i].link).substring(0, 4) == ("http" || "file")) ? (value[i].link) : appPath + "content/" + (value[i].link);
            Element.addEventListener("click", function(){
              document.getElementById("_cover").style.display = "block";
            ...
            var dropDownLoop = function(values, mainC){
              for(var i in values){
                ...
                if(values[i].home == true){
                  ...
                  Element.url = ((values[i].link).substring(0, 4) == ("http" || "file")) ? (values[i].link) : appPath + "content/" + (values[i].link);
                  Element.addEventListener("click", function(){
                    document.getElementById("_cover").style.display = "block";
                    content.loadURL(this.url);
                    closeAllDropdowns();
      ...
      const menuLoop = function(value, MainElement){
        for(var i in value){
          ...
          if(value[i].home == true){
            ...
            var _url = appPath + "content/_main.html", url = _url.split("");
            Element.url = _url;
      ...
      mainBarOptions[mainBarOptions.indexOf("home")] = appPath + "content/_main.html";
