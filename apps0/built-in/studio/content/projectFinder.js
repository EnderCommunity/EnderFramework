...
const COfProjects = document.getElementById("_projects"), { readdirSync } = require('fs'), path_ = require("path"), fs_ = require('fs');
const getDirectories = source => readdirSync(source, { withFileTypes: true }).filter(dirent => dirent.isDirectory()).map(dirent => dirent.name), UpdateProjects = function(){
  var projects = getDirectories(path_.join(__dirname, "\\projects\\")), isThereAnyProjects = false;
  ...
  for(var project in projects){
    ...
    fs_.readFile(path_.join(project, "data.json"), 'utf8', function(error, data){
      if(error){
        ...
        var appInfo = JSON.parse(data), projectC = document.createElement("div"), projectImage = document.createElement("img"), projectD = document.createElement("h5"), projectTitle = document.createElement("h5"), creationTime = document.createElement("text");
